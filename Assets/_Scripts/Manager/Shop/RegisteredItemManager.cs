...
public class RegisteredItemManager : MonoBehaviour
{
    [Header("설정")]
    ...
    public void OnDragEnd(int dropIndex)
    {
        ...
        //등록된 아이템에서 인벤토리로 드롭했을 때 (등록해제)
        if (RegisteredItemTemp.RegiItem.GetDragStartIndex() != -1)
        {
            if (dropIndex == -1) return;

            //드래그한 아이템을 인벤토리에 넣기
            Item registerItem = RegisteredItemTemp.RegiItem.GetDraggedItem();
            int registerCount = RegisteredItemTemp.RegiItem.GetDraggedItemCount();
            int registerIndex = RegisteredItemTemp.RegiItem.GetDragStartIndex();

            //소분 수량이 1보다 많으면 실행
            if (registerCount > 1)
            {
                splitPopup.OpenPopup(registerItem, registerItem.itemName, registerCount, (amount) =>
                {
                    ProcessMoveFromRagisterItem(dropIndex, registerIndex, registerItem, amount);
                });
            }
            else
            {
                //1개면 바로 이동
                ProcessMoveFromRagisterItem(dropIndex, registerIndex, registerItem, 1);
            }
        }
    }

    //등록된 아이템으로 이동
    private void ProcessMoveFromRagisterItem(int dropIndex, int sourceWarehouseIndex, Item registerItem, int warehouseCount)
    {
        if (registerItem == null) return;
        ...
        if (inventoryItem != null && inventoryItem.itemID == registerItem.itemID)
        {
